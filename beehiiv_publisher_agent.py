...
def create_beehiiv_post(
    ...
    # Read the clean inner content HTML from disk (generated by publisher_designer_agent.py)
    # so Beehiiv's Default Template wraps it cleanly with your manual Beehiiv template layout.
    body_content_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output_body_content.html")
    if not os.path.exists(body_content_path):
        body_content_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output_newsletter.html")
        
    try:
        with open(body_content_path, "r", encoding="utf-8") as f:
            body_content_html = f.read()
    except FileNotFoundError:
        return {"error": f"Content HTML not found at {body_content_path}. Run the publisher agent first."}
